...
def get_book_list(index_url):
    while index_url:
        ...
        for a in book_root.xpath('//div[@class="listBox"]/ul/li/a'):
            book_list[a.text_content()] = a.xpath('@href')[0]
        index_url = False
        for a in book_root.xpath('//div[@class="tspage"]/a'):
            if a.text_content() == '下一页':
                index_url = base_url + a.xpath('@href')[0]
